...
The full-width bottom action bar is smartphone-only: it requires both a phone-width viewport and a coarse primary pointer, so narrowing a desktop window keeps the node-attached popup actions. On a phone it remains a blank white surface when nothing is selected. A selected node initially shows only **Rename**, **Add**, and **Remove**. Each command moves the bar into a focused state: a rename form, an add relationship choice followed by a naming form, or a removal confirmation. Creating a child or parent preserves the current selection, so repeated branch creation stays anchored to the original node.
